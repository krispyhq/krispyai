...
function harness(withRtc = true, visitorOnline = true) {
  const objects = new Map<string, { instance: SessionDO; storage: Map<string, unknown>; state: DurableObjectState }>();
  const env = {
        ...
        if (!target) {
          ...
          const state = {
            storage: {
              ...
              setAlarm: async (when: number | Date) => { storage.set("__alarm", when instanceof Date ? when.getTime() : when); },
              deleteAlarm: async () => { storage.delete("__alarm"); },
              getAlarm: async () => storage.get("__alarm") ?? null,
            },
            getWebSockets: (tag?: string) => tag === "call-visitor" && visitorOnline ? [{ send: () => {} }] : [],
  ...
  const post = (route: string, body: object, authorized = false) => worker.fetch(new Request(`https://edge.example.test${route}`, {
    method: "POST",
    headers: { "content-type": "application/json", ...(authorized ? { "x-tenant-sync-secret": env.TENANT_SYNC_SECRET! } : {}) },
  ...
  const register = async (session: string, secret: string, tenant = "acme") => {
    ...
    return stub.fetch("https://do/call/visitor/register", {
      method: "POST", headers: { [DO_INTERNAL_HEADER]: doInternalSecret(env) }, body: JSON.stringify({ secret }),
    });
  };
  return { env, post, register, objects };
}

const operator = (action: string, sessionId = "session-a", id?: string) => ({ tenantId: "acme", sessionId, action, id });
const visitor = (action: string, sessionId = "session-a", visitorSecret = secretA, id?: string, nonce?: string) => ({ tenantId: "acme", sessionId, visitorSecret, action, id, nonce });
...
test("malformed call bodies fail with 400 before tenant or session lookup", async () => {
  const h = harness();
  const malformed: unknown[] = [null, [], { sessionId: 7, action: "invite" }, { tenantId: {}, sessionId: "a", action: "invite" }, { sessionId: "a", action: ["invite"] }, { sessionId: "a", action: "invite", id: {} }, { sessionId: "a", action: "invite", nonce: [] }];
...
test("a grant being signed is rejected when its call ends and a new one is accepted", async () => {
  ...
  expect((await h.post("/api/call", visitor("accept", "session-a", secretA, call.id, nonce))).status).toBe(200);
  ...
  const atRecheck = new Promise<void>((resolve) => { reached = resolve; });
  target.state.storage.get = async <T = unknown>(key: string) => {
    if (key === "call" && ++reads === 2) { reached(); await paused; }
  ...
  try {
    ...
    expect((await h.post("/api/operator/call", operator("end", "session-a", call.id), true)).status).toBe(200);
    ...
    expect((await h.post("/api/call", visitor("accept", "session-a", secretA, replacementId, replacementNonce))).status).toBe(200);
...
test("HTTP+DO flow protects visitor nonce and session; late and ended calls cannot grant", async () => {
  ...
  expect((await h.post("/api/call", visitor("accept", "session-b", secretB, call.id, "wrong"))).status).toBe(403);
  const vStatus = await h.post("/api/call", visitor("status"));
  const { nonce } = await vStatus.json() as { nonce: string };
  expect(nonce).toBeTruthy();
  expect((await h.post("/api/call", visitor("accept", "session-a", secretA, call.id, "wrong"))).status).toBe(403);
  expect((await h.post("/api/call", visitor("grant", "session-a", secretA, call.id))).status).toBe(409);
  expect((await h.post("/api/call", visitor("accept", "session-a", secretA, call.id, nonce))).status).toBe(200);
  ...
  try {
    ...
    expect((await h.post("/api/call", visitor("end", "session-a", secretA, call.id, nonce))).status).toBe(200);
  } finally { globalThis.fetch = originalFetch; }
  expect(deleted).toEqual(["/twirp/livekit.RoomService/DeleteRoom"]);
  expect((await h.post("/api/call", visitor("grant", "session-a", secretA, call.id))).status).toBe(409);
  expect((await h.post("/api/operator/call", operator("grant", "session-a", call.id), true)).status).toBe(409);
  ...
  expect((await h.post("/api/call", visitor("accept", "session-a", secretA, lateCall.id, lateNonce))).status).toBe(409);
  expect((await h.post("/api/call", visitor("grant", "session-a", secretA, lateCall.id))).status).toBe(409);
});
...
test("one DO alarm expires ringing calls and enforces active-call limit with room cleanup", async () => {
  ...
  let attempts = 0;
  try {
    globalThis.fetch = (async () => { attempts++; return Response.json({ code: "unavailable" }, { status: 503 }); }) as unknown as typeof fetch;
    await target.instance.alarm();
    ...
    expect((await h.post("/api/call", visitor("grant", "session-a", secretA, secondCall.id))).status).toBe(409);
    expect((await h.post("/api/operator/call", operator("invite"), true)).status).toBe(503);

    target.storage.set("callCleanupDueAt", Date.now() - 1);
    globalThis.fetch = (async () => { attempts++; return Response.json({}); }) as unknown as typeof fetch;
    await target.instance.alarm();
