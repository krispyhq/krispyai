...
import { CALL_TOKEN_TTL_SECONDS, callRtcAvailable, closeCallRoom, issueCallToken } from "../src/call-token";
...
test("duplicate invites and responses are idempotent; stale ID cannot mutate a new invitation", () => {
  ...
  expect(transitionCall(next.call, "accept", now + 5, id)).toEqual({ ok: false, reason: "no_invitation" });
});
...
test("token is short lived and scoped to one room, identity, and microphone only", async () => {
  ...
  const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(config.apiSecret), { name: "HMAC", hash: "SHA-256" }, false, ["verify"]);
  expect(await crypto.subtle.verify("HMAC", key, Buffer.from(signature!, "base64url"), new TextEncoder().encode(`${header}.${body}`))).toBe(true);
...
test("missing or insecure RTC configuration disables grants", async () => {
  const accepted = transitionCall(invited(), "accept", now + 1, id);
  if (!accepted.ok) throw new Error("expected acceptance");
  expect(callRtcAvailable({ url: "ws://public.example.test", apiKey: "a", apiSecret: "b" })).toBe(false);
  expect(callRtcAvailable({ url: "ws://localhost:7880", apiKey: "a", apiSecret: "b" })).toBe(true);
  expect(await issueCallToken({ url: config.url, apiKey: config.apiKey }, accepted.call, id, "visitor", now + 1)).toBeNull();
});
...
test("room deletion calls LiveKit with a signed server grant and the exact room", async () => {
  ...
  const mockFetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    const payload = JSON.parse(Buffer.from(headers.authorization!.split(".")[1]!, "base64url").toString());
    expect(payload.video).toEqual({ roomCreate: true });
    return Response.json({});
  });
  expect(await closeCallRoom(config, "private-room", mockFetch)).toBe(true);
  expect(seen).toBe(true);
  expect(await closeCallRoom(config, "private-room", async () => Response.json({ code: "not_found" }, { status: 404 }))).toBe(true);
  expect(await closeCallRoom(config, "private-room", async () => new Response("wrong endpoint", { status: 404 }))).toBe(false);
});

test("SessionDO keeps invitation nonce from operators and requires visitor capability", async () => {
  const storage = new Map<string, unknown>();
  const env = { DO_INTERNAL_SECRET: "test-do-secret", LIVEKIT_URL: config.url, LIVEKIT_API_KEY: config.apiKey, LIVEKIT_API_SECRET: config.apiSecret } as Env;
  const state = {
    storage: {
  ...
  const baseHeaders = { [DO_INTERNAL_HEADER]: doInternalSecret(env), "content-type": "application/json" };
  const req = (path: string, body: object, headers: Record<string, string> = {}) => instance.fetch(new Request(`https://do${path}`, {
    method: "POST", headers: { ...baseHeaders, ...headers }, body: JSON.stringify(body),
  }));
  ...
  expect(call.status).toBe("ringing");
  const opStatus = await instance.fetch(new Request("https://do/call", { headers: { ...baseHeaders, "x-call-actor": "operator" } }));
  expect(await opStatus.json()).not.toHaveProperty("nonce");
  const denied = await req("/call", { action: "accept", id: call.id }, { "x-call-actor": "visitor", "x-call-visitor-secret": "b".repeat(43) });
  expect(denied.status).toBe(403);
  const visitorStatus = await instance.fetch(new Request("https://do/call", { headers: { ...baseHeaders, "x-call-actor": "visitor", "x-call-visitor-secret": secret } }));
  const { nonce } = await visitorStatus.json() as { nonce: string };
  expect(nonce).toMatch(/^[0-9a-f-]{36}$/);
  expect((await req("/call", { action: "accept", id: call.id, nonce: "wrong" }, { "x-call-actor": "visitor", "x-call-visitor-secret": secret })).status).toBe(403);
  expect((await req("/call", { action: "accept", id: call.id, nonce }, { "x-call-actor": "visitor", "x-call-visitor-secret": secret })).status).toBe(200);
  expect((await req("/call/grant", { id: call.id }, { "x-call-actor": "visitor", "x-call-visitor-secret": secret })).status).toBe(200);
  const fetchBeforeEnd = globalThis.fetch;
  try {
    globalThis.fetch = (async () => Response.json({})) as unknown as typeof fetch;
    expect((await req("/call", { action: "end", id: call.id, nonce }, { "x-call-actor": "visitor", "x-call-visitor-secret": secret })).status).toBe(200);
  } finally { globalThis.fetch = fetchBeforeEnd; }
  expect((await req("/call/grant", { id: call.id }, { "x-call-actor": "visitor", "x-call-visitor-secret": secret })).status).toBe(409);
});
