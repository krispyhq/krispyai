// @krispy/api — Hono + @hono/zod-openapi.
//   GET  /health          liveness
...
//   CRUD /posts            example resource, zod-validated, backed by @krispy/db
import { OpenAPIHono } from "@hono/zod-openapi";
import { swaggerUI } from "@hono/swagger-ui";
import { cors } from "hono/cors";
import { auth } from "@krispy/auth";
import { db, user, eq } from "@krispy/db";
import { getEnv } from "@krispy/config";
...
// comes from the typed env door (@krispy/config) — declared once, no inline default here.
...
app.doc("/openapi.json", {
  openapi: "3.1.0",
  info: { version: "1.0.0", title: "@krispy/api", description: "Builders-stack reference API." },
});
...
// PORT is injected by portless in local dev (stable URL: api.krispy.localhost:1355);
// falls back to 3001 for standalone `bun --filter @krispy/api dev`.
const port = Number(process.env.PORT) || 3001;
console.log(`[api] listening on http://localhost:${port}  (docs: /docs)`);

export default { port, fetch: app.fetch };

// The full app type — the second way to stay in sync with the frontend. @krispy/api-types
