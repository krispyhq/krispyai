...
// backed by @krispy/db (Drizzle). This is the pattern to copy for real resources.
import { createRoute, z } from "@hono/zod-openapi";
import { db, posts, eq } from "@krispy/db";
import type { Post, NewPost } from "@krispy/db";
// The request/response schemas live in @krispy/api-types — the ONE contract shared
// with apps/web. Here they drive validation + OpenAPI; there they drive typed calls.
import { PostSchema, NewPostSchema, PatchPostSchema, IdParam, ErrorSchema } from "@krispy/api-types";
...
// ---- data access (idiomatic Drizzle; @krispy/db provides db + schema) ----
