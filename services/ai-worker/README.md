...
Background job runner — **no HTTP server**. Drains a queue and runs jobs that call `@krispy/ai`.

```bash
bun --filter @krispy/ai-worker dev     # starts the loop; Ctrl-C for graceful stop
```
...
- **`index.ts`** — one sample job (`summarize`) calling `@krispy/ai`, plus
