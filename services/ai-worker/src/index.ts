// @krispy/ai-worker — background job runner (no HTTP server).
// Drains a queue and runs jobs that call @krispy/ai. Graceful start/stop.
...
async function handleSummarize(data: SummarizeJob, job: { id: string; attempt: number }) {
  ...
  // in a real worker: persist the summary via @krispy/db here.
}
...
if (process.env.AI_WORKER_DEMO !== "0") {
  queue.add({
    postId: "p_demo",
    text: "The krispyai is a reference monorepo for shipping fast.",
  });
}
