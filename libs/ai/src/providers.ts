...
export function getModel(provider: Provider, modelId: string, apiKey?: string): LanguageModel {
  ...
  if (!make) {
    throw new Error(
      `@krispy/ai: unknown provider "${provider}". Expected one of: ${Object.keys(factories).join(", ")}`,
    );
  }
...
export function resolveApiKey(explicit?: string): string {
  ...
  if (!key) {
    throw new Error(
      "@krispy/ai: no API key. Set the AI_API_KEY env var or pass `apiKey` to the call.",
    );
  }
  return key;
}
