...
[Better Auth](https://better-auth.com) wired to the `@krispy/db` Drizzle client
...
```ts
import { auth, getSession, type Session, type User } from "@krispy/auth";
...
| `DATABASE_URL`                              | Postgres connection — read by `@krispy/db`, not here.       |

## Generating the auth schema

Better Auth owns its tables (`user`, `session`, `account`, `verification`).
Rather than hand-writing them into `@krispy/db`, generate them from this config:
...
Drizzle schema to `../db/src/auth-schema.ts`. Re-export it from `@krispy/db`'s
