...
| Import                 | What you get                                                          |
| ---------------------- | --------------------------------------------------------------------- |
| `@krispy/ui`            | Components + `cn` (web; pulls in Radix/DOM).                          |
| `@krispy/ui/tokens`     | Framework-agnostic tokens — **safe for React Native** (no DOM/React). |
| `@krispy/ui/styles.css` | The Tailwind v4 stylesheet (import once in a web app).                |
...
```css
/* app/globals.css */
@import "@krispy/ui/styles.css";
```

```tsx
import { Button, Card, CardHeader, CardTitle, cn } from "@krispy/ui";
...
Next.js consumes the TypeScript source directly, so add `@krispy/ui` to
...
```ts
import { colors, spacing, radii } from "@krispy/ui/tokens";
