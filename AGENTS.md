...
- **build** — `apps/docs` **static-exports** (`next.config.mjs` `output: 'export'` → `out/`) and the deploy just uploads `out/` to CF Pages — no `@cloudflare/next-on-pages`, no per-route `runtime = 'edge'`, no Node/edge runtime. All docs routes are SSG; search is a build-time Orama index served as a static file at `/api/search` (`app/api/search/route.ts` exports `staticGET`, `app/layout.tsx` sets `search.type: 'static'` so the dialog fetches the static index client-side). The widget is static (no build); the edge Worker is deployed straight from `src` by wrangler.
...
- **smoke** (`scripts/cf-deploy-smoke.mjs`) — edge: `GET /health` = 200 `{status:"ok"}`; pages: `GET /` = 200, and for the docs site also `GET /api/search` = 200 JSON (the static search index). Skipped (with a warning) until you record the deployed URL in `.env.local` (`EDGE_PRODUCTION_URL`, …).
...
- **Custom domains (`krispyai.com`)** — attached in the CF dashboard **after the first deploy**, once the `krispyai.com` NS transfer to Cloudflare completes (the zone must exist in CF first). Prod hostnames: **edge.krispyai.com** (Workers → Settings → Domains), **docs.krispyai.com** + **widget.krispyai.com** (each Pages project → Custom domains). These are the smoke-check defaults in `deploy.sh`; preview stays on the `workers.dev`/`pages.dev` URL (set `EDGE_PREVIEW_URL` etc. in `.env.local` to smoke it). For a Pages **preview** deploy the smoke URL is the **branch alias** `https://preview.<project>.pages.dev` (e.g. `DOCS_PREVIEW_URL=https://preview.krispy-docs-preview.pages.dev`), **not** the bare `<project>.pages.dev` — that hostname serves the (empty) production branch and 404s until a production deploy exists. Runtime source never hardcodes a hostname — domains live in wrangler/CF config + smoke URLs only.
