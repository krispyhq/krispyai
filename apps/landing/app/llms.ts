...
// (see @krispy/seo + the pages themselves), not this file.
...
- [One design system](${SITE_URL}/): \`@krispy/ui\` ships shadcn components plus framework-agnostic design tokens. Web and React Native render the exact same brand from one token source of truth.
...
- \`libs/\` — shared code imported by package name (never a deep path): \`@krispy/ui\`, \`@krispy/db\`, \`@krispy/auth\`, \`@krispy/ai\`, \`@krispy/analytics\`, \`@krispy/email\`.
Nx enforces the dependency boundaries between these layers, so the structure can't silently rot as the repo grows.

### Design system
\`@krispy/ui\` is the one component + token layer. Components are shadcn/ui (Radix + Tailwind). Tokens live behind a framework-agnostic \`@krispy/ui/tokens\` entry (plain data — no DOM, no React) so React Native consumes the exact same color/spacing/typography values the web does. The web CSS variables are mirrored from that token file, which is the single source of truth.
...
### Local dev
Local services run through portless, which assigns stable named URLs (e.g. web.krispy.localhost) with no pinned ports. One command brings the whole stack up.
