...
`@krispy/ui`'s `tokens` — proof the design system is shared across web _and_ native.
...
bun --filter @krispy/mobile start   # or: cd apps/mobile && bun run start
```
...
config.resolver.unstable_enablePackageExports = true; // so @krispy/ui/tokens resolves
```
...
## Contract with `@krispy/ui` (built in parallel)

- Imports `import { tokens } from "@krispy/ui/tokens"` — a **pure** subpath: no React, no DOM,
  no shadcn component code (native can't bundle those). Keep the barrel (`@krispy/ui`) for web.
