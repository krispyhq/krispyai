...
bun --filter @krispy/web dev      # → http://localhost:3000
```
...
| Route     | Proves                                                                                                                              |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `/`       | The design system — several `@krispy/ui` components + the `tokens` swatch row (same tokens native uses). Plus the live session card. |
| `/health` | Cross-role wiring — server-side `fetch` to `@krispy/api`'s `/health`, rendered.                                                      |
| `/auth`   | Better Auth email/password sign-in + sign-up against `@krispy/api`.                                                                  |

## How it's wired

- **Design system** — `import { Button, Card, Badge, Input, Label, tokens } from "@krispy/ui"`. `next.config.ts` sets `transpilePackages: ["@krispy/ui"]` so the lib compiles from TS source, no build step.
- **Tailwind v4** — theme lives in `app/globals.css` (`@import "tailwindcss"` + `@theme`). The `@source "../../../libs/ui/src"` line makes Tailwind scan the lib so utility classes used _inside_ `@krispy/ui` components get generated. CSS variables here mirror the JS `tokens` export — one source of truth, two shapes (CSS for web, JS for native).
...
**`@krispy/ui`** must export (import by package name):
...
- `tokens`: a plain JS object with `colors.light` / `colors.dark` (hex values), `brand`, `spacing`, `radii`, `typography` — also exposed as the pure `@krispy/ui/tokens` subpath for native.

**`@krispy/api`** must serve, on `http://localhost:3001`:
