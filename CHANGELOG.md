...
- Edge/widget: **pre-chat identification gate** — **off by default.** A new optional `TenantConfig.identify` (`IdentifySpec`: `require: "email" | "none"`, `title?`, `description?`, `collectName?`) lets a self-host keep the chat open to anonymous visitors while requiring an address before the first message, so an answer can still reach someone who leaves. With it unset (or `require: "none"`) the key is **absent from `GET /api/widget/config` entirely** and `POST /api/chat` gates nothing — an existing embed's boot JSON and chat behavior are byte-for-byte unchanged (asserted by test). When required, `POST /api/chat` answers `403 { error: "identity_required" }` for a turn with no usable identity, **before the model runs**: the widget renders the card but the **edge** is the gate. `identity` (`{ email, name? }`) rides the existing chat body and is re-validated server-side every turn (trimmed, lowercased, ≤200 chars, dotted domain required — stricter than `<input type="email">`, which accepts `a@b`); anything malformed counts as absent. The first valid identity is stored **write-once** on the session's `SessionDO` via the `POST /context` body that already runs each turn (no new route, no extra subrequest), so later turns pass without it. Deliberately **not** wired into `/api/lead`: the gate fires before a topic exists, so the Telegram half of the lead fan-out would be a no-op and the email half would mail a lead for someone who has not spoken yet.
- Edge: the captured identity reaches the operator where they actually reply — posted as the **first message of the visitor's Telegram topic** (`✉️ <name · >email`, silent + best-effort, distinct from the `👤` that prefixes visitor messages) and returned on `POST /api/operator/handoffs` inbox rows. `SessionDO` `POST /context` and `GET /summary` now return `identity` when a session has one. It is never written to KV, emailed, or sent to a connector — see `docs/security.mdx` for the full inventory of what is stored and where.
- Edge: `POST /api/tenant/config` write caps for the gate — `400 identify_require_invalid` (an unknown `require` is rejected rather than silently treated as off) and `413 identify_text_too_large` (`title`/`description` over 500 chars, the same bound as the other free text projected to every visitor).
- Widget: the gate reuses the existing lead-form card renderer (`showForm` gained an optional submit handler + an optional `description` line) rather than adding a second form engine. While the gate is up the composer is disabled, starter chips are withheld, and the card lands after any scripted opener; on submit the address unlocks the composer and is kept in `localStorage["krispy_identity_<tenant>"]` so a returning visitor isn't re-asked. It re-sends on every chat post, so a `SessionDO` hiccup can't lock a visitor out — and a `403 identity_required` raises the card even when the boot config said nothing, so the server always wins.

- Edge: **kbase injection** — `TenantConfig.kbSources` (`KbSource[]`: `{ id, name, text, updatedAt }`, total text still hard-capped at 100K chars) is now assembled into the system prompt at chat time as a `## Knowledge` block (runtime assembly, not compile-on-save), between the persona and the forms/guardrail contracts. Unset → the prompt is byte-for-byte what it is today. `detectPromptLeak` is **narrowed to the instruction portion** (the same prompt without the knowledge block) so a bot quoting its own KB verbatim is no longer false-flagged as a prompt leak and forced into a handoff.
- Edge: **relearning from handoffs** — **off by default:** it runs only for a tenant already using the knowledge base (≥1 `kbSource`), so a tenant that never configured a KB incurs zero billable AI on handback and behaves exactly as today. When a handed-off session hands back to the AI (operator resolves it, or the silence alarm fires) and it had ≥1 operator message, the `SessionDO` makes one Workers-AI call (metered under `ai`) to extract at most one Q→A the human answered that the bot couldn't, and appends it as a `KbSuggestion` (`{ id, question, answer, createdAt }`) under its **own** per-site KV key `suggestions:<ns(t,s)>` (cap 20 FIFO; normalized-question dedup against pending suggestions **and** approved `kbSources`). A separate key means the background write can never race a human config save. Failures are swallowed — relearning never breaks resolve. The DO now persists its `tenantId`+`siteId` write-once on the `POST /context` read (the handback fires from an alarm with no request in flight and the DO can't derive them from its own name). `TenantConfig.kbVersion` bumps on any `kbSources` write.
...
- CLI: `krispy logo <file>` — removes a logo's background locally (no cloud, no API key) via corner chroma-key and prints a paste-ready `data:image/png;base64,…` avatar URI for `theme.avatar` (self-host parity for the cloud dashboard's AI bg-removal). Handles png/jpg/webp/svg, downscales to 144px; leaves already-transparent or busy-background images untouched. Depends on `sharp` (lazy-loaded so the CLI core stays dep-free at import).
