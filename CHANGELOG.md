...
- Edge: **knowledge retrieval** — **off by default, and unset it changes nothing:** with no `KNOWLEDGE_PROVIDER` (or no `IMMORTERM_MEMORY_URL`) a null adapter is wired, there is no network call and no new dependency, and the assembled system prompt is byte-for-byte what it is today (asserted by test). Configured, and with a tenant opting in via the new `TenantConfig.knowledgeEnabled`, each `/api/chat` turn reads two things **in parallel** before the completion — a pre-baked KB digest (`## Knowledge base summary`) and the top-K chunks for that message (`## Relevant to this question`) — and injects both **additively**: `systemPrompt` and `kbSources` still ride every turn, so a hand-written brand voice is never evicted by retrieved text. No tool-calling, so it stays **one model call per turn**. `detectPromptLeak` treats retrieved text like `kbSources` (excluded from the leak scope), so a bot quoting a retrieved chunk isn't false-flagged as a prompt leak and force-handed-off. The digest is cached in KV under the versioned key `ram:<tenant>[:<site>]:<kbVersion>`, so a `kbVersion` bump invalidates it for free — no purge call, no purge race.
- Edge: the knowledge seam is an **interface, not a vendor** (`services/edge/src/knowledge.ts`) — `Knowledge` + a null adapter + an ImmorTerm sidecar adapter, so a different backend is a drop-in and the chat path never imports a client. Both methods are total: they resolve to `""` / `[]` on a miss, a bad response, a timeout, or a dead provider, and a KV failure degrades the same way. Retrieval can only ever **remove** context — never block, break, or slow a turn.
- Edge: pack handles are **derived from the authenticated tenant**, never supplied by a request (`tenant-<sha256(ns(tenant,site))>`, non-reversible, one pack per site). The sidecar ships no auth and its search takes an arbitrary pack name, so forwarding a client-supplied one would be a cross-tenant KB read; a test asserts a forged pack name in the request body cannot reach the provider. `IMMORTERM_ACCESS_CLIENT_ID`/`_SECRET` (secrets) send a Cloudflare Access service token to a tunnelled sidecar.
- Edge: new optional vars, all with code defaults and all documented commented-out in `wrangler.toml` (in **all three** env blocks — named envs don't inherit vars): `KNOWLEDGE_PROVIDER`, `IMMORTERM_MEMORY_URL`, `KNOWLEDGE_SEARCH_LIMIT` (3), `KNOWLEDGE_RAM_TTL_S` (300), `KNOWLEDGE_RAM_TIMEOUT_MS` (400), `KNOWLEDGE_SEARCH_TIMEOUT_MS` (600).
- Docs: [knowledge retrieval](https://docs.krispyai.com/docs/guides/knowledge-retrieval) — the size gate (below ~20K chars, keep inlining and don't turn this on), setup, the security warning about exposing a no-auth sidecar, the tuning knobs, and the full degradation table.
