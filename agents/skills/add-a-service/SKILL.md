...
1. **Name it.** `services/<name>`, package `@krispy/<name>`. **No pinned port** — served roles run behind Portless at `<name>.krispy.localhost:1355` (Portless injects a random `PORT`). See `docs/portless.md`.
...
6. **Persistence via `@krispy/db` only.** Payments via `@krispy/payment` only. Never import another service's internals — talk over HTTP.
...
7. **Wire it into `.devops/Tiltfile`** (the runtime manifest — this is required, not optional). Wrap served roles with the `portless_cmd` helper so they get a stable named URL; a background worker skips Portless:
   ...
   ```python
   # served role — gets <name>.krispy.localhost:1355 via portless
   local_resource('<name>',
     serve_cmd='cd ' + REPO_ROOT + ' && ' + ENV_LOCAL + portless_cmd(
       '<name>.stack ' + BUN + ' --filter @krispy/<name> dev',
...
`./tilt_up.sh` shows the new resource green in the dashboard (`localhost:10380`); `curl http://<name>.krispy.localhost:1355/health` returns `{ "ok": true }` (skip for a worker — check its log line instead). `bun run typecheck` passes.
