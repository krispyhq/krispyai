...
Always **`./tilt_up.sh`, never `tilt up` directly** — the script pins a per-project Tilt UI port (10380) so multiple Tilt projects coexist. Served roles get stable **portless** URLs (no pinned ports): `web.krispy.localhost:1355`, `api.krispy.localhost:1355`, `payment.krispy.localhost:1355`, `storybook.krispy.localhost:1355`. See `docs/portless.md`.
...
2. **One public door** — each lib exposes a single `src/index.ts`; import by package name (`@krispy/db`), never a deep path.

Plus: **by feature, not by layer** inside each package · **one ORM (Drizzle) via `@krispy/db`** · **payments via the `@krispy/payment` adapter** · every workspace extends `tsconfig.base.json` · **no hardcoded URLs/ports/secrets** (use env; add new vars to `.env.example`).
