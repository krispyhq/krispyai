...
for (const rel of pageGlob.scanSync({ cwd: APPS_DIR, onlyFiles: true })) {
  ...
  if (!hasMetadataExport(src)) {
    violations.push(
      `${path}\n    → public page must export \`metadata\` or \`generateMetadata\` (use @krispy/seo's pageMetadata()).`,
    );
  }
...
if (violations.length > 0) {
  ...
  console.error(
    "Laws: AGENTS.md § “SEO/GEO — enforced”. The one door is @krispy/seo — don't hand-roll.\n",
  );
  process.exit(1);
}
