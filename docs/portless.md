...
- **Human-readable URLs** — `api.krispy.localhost:1355` instead of `localhost:3001`.
...
```bash
portless api.stack bun --filter @krispy/api dev
# Portless injects PORT=4xxx, proxies api.krispy.localhost:1355 → localhost:4xxx
```
...
Storybook binds the port from its `-p` flag, so `@krispy/ui`'s script passes portless's injected `PORT` straight through:

```json
"storybook": "storybook dev -p ${PORT:-6006} --no-open"
```

`portless storybook.stack bun --filter @krispy/ui storybook` then proxies `storybook.krispy.localhost:1355` → that port. Standalone (no portless) it falls back to 6006.
