...
Each service has a Dockerfile under `infra/`. Build from the repo root (the build context is the whole monorepo so bun can resolve `@krispy/*` workspaces):
...
```bash
bun --filter @krispy/db migrate
```

## 4. Roll out to Kubernetes

`infra/k8s/deployment.yaml` is a worked example for one service (`@krispy/api`) — a `Deployment` with `readinessProbe`/`livenessProbe` on `/health` and env pulled from a `stack-secrets` Secret. Copy it per service, swap the image, and apply:
...
## Frontend app → Cloudflare Workers (OpenNext)

`apps/web` (the dashboard) is Next.js — deploy it to **Cloudflare Workers** with the [OpenNext Cloudflare adapter](https://opennext.js.org/cloudflare) (`@opennextjs/cloudflare`, 1.0 GA since Feb 2026; Next.js 14/15/16).

From the app dir (`apps/web`):
...
### Marketing site (landing + blog) — moved to the `krispy-site` repo

The public **landing** page and the **blog** used to live here (`apps/landing`, `apps/blog`) and deployed via the same OpenNext → Cloudflare Workers path (blog on its own `blog.<domain>` subdomain). They now live in their own repo — [github.com/lonormaly/krispy-site](https://github.com/lonormaly/krispy-site) — and deploy from there. Their `cf:deploy` scripts, `wrangler.toml`s, and the `NEXT_PUBLIC_APP_URL` → dashboard link all moved with them; nothing in this repo deploys them anymore.
