...
| ----------- | ------------------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------ |
| `apps/`     | what **humans** see             | public UI                        | `@krispy/web` (Next.js), `@krispy/mobile` (React Native)                                           |
| `services/` | what has a **URL** / own deploy | served to other code             | `@krispy/api` (Hono + OpenAPI), `@krispy/ai-worker` (background), `@krispy/payment` (Creem adapter) |
| `libs/`     | **shared** code                 | **never served** — consumed only | `@krispy/ui`, `@krispy/auth`, `@krispy/db`, `@krispy/ai`                                             |
...
flowchart TD
    subgraph apps["apps/ — public UI"]
      web["@krispy/web"]
      mobile["@krispy/mobile"]
    end
    subgraph services["services/ — has a URL"]
      api["@krispy/api"]
      worker["@krispy/ai-worker"]
      payment["@krispy/payment"]
    end
...
Each lib exposes a single `src/index.ts`. Consumers import by **package name** (`@krispy/db`), never a deep path (`@krispy/db/src/schema/users`).

```
import { users } from "@krispy/db";              ✅  the public door
import { users } from "@krispy/db/src/schema";    ❌  reaching past it
```
