...
| ------------- | ------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **CC6.1**     | Logical access — authentication | ✅     | Better Auth (`@krispy/auth`) — email/password + GitHub OAuth, sessions in Postgres.                                                                                             |
...
| **CC6.6**     | Boundary protection             | 🔨     | CORS pinned to `WEB_ORIGIN` (`@krispy/config`); Nx module boundaries. Add WAF/rate limits.                                                                                      |
...
- **Audit logging** — `securityEvent(actorId, event, props)` in `@krispy/analytics/events`:
