...
2. **One public door** — each lib exposes a single `src/index.ts`, mapped in `tsconfig.base.json`. Callers import `@krispy/db`, never `@krispy/db/src/internal/…`.
...
3. **Name it** in its `package.json`: `"name": "@krispy/db"`, and declare the door in `exports`:
   ```jsonc
   {
     "name": "@krispy/db",
   ...
   ```
4. **Map the path** in `tsconfig.base.json` — the `./`-prefixed entry (there's no `baseUrl`; a bare `libs/…` throws TS5090, and Nx needs this path to resolve `@krispy/*` for the boundary rule):
   ```jsonc
   "paths": { "@krispy/db": ["./libs/db/src/index.ts"] }
   ```
...
Then rewrite the imports at the call sites: `../../lib/db` becomes `@krispy/db`. Because everything now enters through one door, that's a flat find-and-replace, not a path-chasing exercise.
...
```python
portless_cmd("api", "bun --filter @krispy/api dev", port_env="PORT")
```
...
import { db } from "../../lib/db"   →   import { db } from "@krispy/db"
```
...
- **Swap the plumbing:**
  ...
  - Rename your scope to `@krispy/*` — **or keep your own** (`@yourco/*`) and update the `paths` keys / Tiltfile `--filter` targets accordingly. The [rename sweep in `make-it-yours.md`](./make-it-yours.md#rename-stack--yourco) is a one-liner either direction.
...
- **Carve in coupling order, lowest first:**
  ...
  2. The **payment / billing** surface (naturally isolated behind an adapter — see [`make-it-yours.md`](./make-it-yours.md) on the `@krispy/payment` pattern).
...
- **Rename `@krispy/*` → `@yourco/*`** — because of one-public-door + import-by-name, it's a flat find-and-replace with no scattered internal paths to chase.
