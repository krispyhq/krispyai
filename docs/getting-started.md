...
The turnkey guide to taking krispyai from a fresh clone to a running app with the integrations you want on. **A fresh clone boots on an empty `.env.local`** — every paid integration is env-gated to a silent no-op (no key → the feature is off, the app still runs). You add keys only when you want that feature.
...
You now have web + landing + api + payment (Mock provider) + storybook running at `*.krispy.localhost:1355`. Login, email, payments, analytics, and AI are all off until you add their keys.
...
There's **one** env file: root `.env.local`. `./tilt_up.sh` sources it into every service automatically. But standalone `bun --filter @krispy/<x> dev` only loads a `.env.local` from the _invoking_ directory, so **`./scripts/link-env.sh`** symlinks the root file into each app/service (`ln -sf ../../.env.local <pkg>/.env.local`). Re-run it any time; the symlinks are gitignored. Edit keys in one place: root `.env.local`.
