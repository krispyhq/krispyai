...
- **Conversation runtime settings** (`AUTO_ARCHIVE_HOURS`, `BUTTR_INBOX_URL`) use the same optional per-key sync. The archive window defaults to 24 hours when unset. Set the inbox URL to the authenticated operator inbox for the target environment so lead-email links open a tenant-checked conversation; keep preview and production URLs in their own Infisical environments.
