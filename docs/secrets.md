...
- **Optional gateway configuration** (`KNOWLEDGE_GATEWAY_URL`, `KNOWLEDGE_TENANT_ID`, `KNOWLEDGE_SITE_ID`, `KNOWLEDGE_TIMEOUT_MS`) is synced from the Infisical-fed `.env.local` by `scripts/sync-edge-secrets.mjs` as plain-text Worker bindings. Missing keys remove these optional bindings, so the gateway stays disabled.
