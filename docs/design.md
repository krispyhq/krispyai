# Designing UI in krispyai

The stack: **shadcn/ui** (Radix primitives + Tailwind) in `@krispy/ui` — every component's source lives in-repo, so your coding agent can read and edit it. Icons: `lucide-react`. Tokens (`@krispy/ui/tokens`) are the single source of truth, consumed by both web and native.
...
3. **Build** — implement as `@krispy/ui` components (shadcn + `cn()` + `cva`), composed by the app. Reusable → a component in `@krispy/ui`; one-off page scaffolding → inline in the app.
...
- Reusable UI = a `@krispy/ui` component. Never inline reusable UI or duplicate styles inside an app.
- Compose in apps; define in `@krispy/ui`. Screens hold layout + data wiring + composition — not reusable UI.
